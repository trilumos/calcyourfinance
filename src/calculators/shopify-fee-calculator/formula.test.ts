...
describe("shopify-fee-calculator", () => {
  it("Basic + Shopify Payments (2.9% + $0.30): $100 → fee $3.20, keep $96.80", () => {
    const r = computeMarketplaceFee({ itemPrice: 100, sellingPercent: 0, processingPercent: 2.9, processingFixed: 0.3 });
    expect(r.totalFees).toBe(3.2);
    expect(r.payout).toBe(96.8);
  });

  it("Advanced + Shopify Payments (2.5% + $0.30): $100 → fee $2.80", () => {
    const r = computeMarketplaceFee({ itemPrice: 100, sellingPercent: 0, processingPercent: 2.5, processingFixed: 0.3 });
